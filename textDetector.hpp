...
void writeImage(string folderName, int temporalCounter, int fps, int auxTime, int counter, cv::Mat temporalFrameColor)
{
...
void imgExtractor(const std::string &fileName)
{
    ...
    cout << "Abriendo el video: " + fileName << endl;
    cv::VideoCapture video(pathVideo);
    if (!video.isOpened())
    {
    ...
    cout << "FPS del video: " + to_string(fps) << endl;
    ...
    while (counter <= total_frames)
    {
        ...
        if (counter % fpsCut == 0)
        {
            ...
            {
                writeImage(folderName, temporalCounter, fps, auxTime, counter, temporalFrameColor);
